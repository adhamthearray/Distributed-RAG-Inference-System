...
## 2. Configuration
Runtime configuration lives in the root `.env` file. Edit that file to change the Groq API key and worker counts used by the app and Docker Compose. Use `.env.example` as the template if you need to recreate it.

## 3. Worker Error Handling
If the LLM call fails, the worker returns that execution error to the caller. The Master does not retry, reassign, or recover the task.

## 4. RAG Pipeline & Vector DB Optimizations
...
* **Validation**: This script validates that Nginx distributes the load, the Master Node queues the tasks properly, and the Workers execute them in parallel.
